...
* experiment - contains python scripts used to conduct the experiments reported in the paper
* util - contains some utility functions used by other subpackages
...
## Replicating Experiments and Examples in the Paper

The following replicates the random BT experiments:

```
python random_tests.py      # Executes the random behaviour trees, not necessary if logs already generated
python random_explain.py    # Generates explanations for differences in the executions
```

The following replicates the serial recall experiments:

```
./run_multi.sh              # Executes the serial recall behaviour trees, not necessary if logs already generated
./evaluate_cog.sh           # Generates explanations for differences in the executions
```

The following replicates the LLM baseline experiments:

```
./evaluate_llm.sh           # Generates explanations for differences in the executions (requires serial recall logs)
```

Please edit *evaluate_llm.sh* to select between models and prompts.
...
Please edit *run_case_study.py* to select between case study queries.

## Places of Interest
The episodic memory logs generated by the BT executions are all contained in the [logs directory](logs/README.md).

The results of the experiments are all contained in the [results directory](results/README.md).

Examples of prompts provided to the LLMs are given in the [example prompts directory](btcm/experiment/example_prompts/README.md).

Details on the behaviour trees used are given in the [cognitive sequence](btcm/examples/cognitive_sequence/README.md) and [random BT](btcm/examples/random/README.md) directories.
